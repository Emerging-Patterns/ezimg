...
| IMG-PNG-7 | For every byte list `xs` and every block size from 1 to 65535, `Inf.inflate` of the zlib stream of `xs` in stored blocks of that size is `Some(xs)`. | Proved | pending | LAWS.bend inflate_stored; LAWS.bend inflate_enc_zlib |
...
| ID | Requirement | Level | Status | Law |
| :---- | :---- | :---- | :---- | :---- |
| IMG-JPG-1 | `decode_jpeg` returns none for every input whose first frame header is not SOF0, and for every SOF0 frame whose sample precision is not 8. | Proved | pending | LAWS.bend jpeg_refuse_sofn; LAWS.bend jpeg_refuse_precision |
...
| IMG-JPG-5 | When `encode_jpeg(r)` is some, it begins with SOI, APP0 with the JFIF identifier, DQT, SOF0 carrying `r`'s width and height, DHT and SOS, and ends with EOI. | Proved | pending | LAWS.bend jpeg_enc_layout; LAWS.bend jpeg_enc_ends |
...
| :---- | :---- | :---- |
| IMG-PIX-1 | The JPEG side. `Jpeg.rgb` and `Jpeg.gray` give alpha 255 for every input (`jpeg_rgb_opaque`, `jpeg_gray_opaque`); every sample `decode_jpeg` returns is packed by `Jpeg.gray`, or every one by `Jpeg.rgb` (`jpeg_decode_packed`); every sample it returns has alpha 255 (`jpeg_decode_opaque`); a gray sample carries its level's low byte in R, G and B (`jpeg_gray_level`) | that every sample `decode_png` returns is `0xAARRGGBB` (IMG-PNG-9). `jpeg_decode_packed` does not say that the gray packing is the one chosen for a one-component frame |
| IMG-JPG-1 | Over a stream builder: bytes that open with SOI and segments that are not frame headers (DHT, DQT, SOS, DRI, COM, APP0 to APP15, each with a length field that fits its body), then a frame header other than SOF0 (SOF1 to SOF3, SOF5 to SOF7, SOF9 to SOF11, SOF13 to SOF15), decode to none whatever follows (`jpeg_refuse_sofn`); the same with an SOF0 segment whose precision byte is not 8 (`jpeg_refuse_precision`) | the row over arbitrary bytes: a segment whose length field does not fit its body, or a marker without a length (RSTn, TEM, a second SOI), before the frame header. A rewording to the builder's terms is proposed |
| IMG-JPG-5 | For a raster with both sides at most 65535: SOI, APP0 opening with `JFIF` and a zero byte, DQT, SOF0 opening with precision 8 and `r`'s height and width, two DHT, SOS, the entropy-coded bytes and EOI (`jpeg_enc_layout`). For every raster: SOI first and EOI last (`jpeg_enc_ends`) | the row is false as worded for a side above 65535: `encode_jpeg` of a 65536 by 1 raster is `Some([255, 216, 255, 217])`, SOI then EOI with no APP0, DQT, SOF0, DHT or SOS. A decision is pending |
| IMG-PNG-7 | every byte list `xs` (values 0 to 255) shorter than 2^32 bytes, which its U32 count `Png.enc.len(xs, 0)` states without wrapping, in the zlib stream `zlib.stored(sz, xs)` ezimg writes (120 1, `Png.enc.blocks(sz, xs)`, the big-endian Adler-32 of `xs`) for every block size `sz` from 1 to 65535, inflates to `Some(xs)` (`inflate_stored`); so does `Png.enc.zlib`, the stream `encode_png` writes when the scanlines fit one block (`inflate_enc_zlib`) | lists of 2^32 bytes or more, for which the row is false as worded: `Png.enc.blocks` counts `xs` in a U32, and a count that wraps to at most the block size writes one block whose LEN is the wrapped count. Worded for lists shorter than 2^32 bytes, the row is proved |
...
Every Proved row but IMG-JPG-4, IMG-PIX-2, IMG-RAS-5, IMG-PNG-1, IMG-PNG-5, IMG-PNG-6 and IMG-PNG-8 is pending; IMG-PIX-1, IMG-JPG-1, IMG-JPG-5 and IMG-PNG-7 have the partial laws above. The rollout in [docs/rfc/ezimg-spec.md](docs/rfc/ezimg-spec.md) orders them: the behavior changes first (IMG-PIX-1 needed BC-1, which has landed; IMG-JPG-2 needed BC-2 and IMG-JPG-4 needed BC-3, which have landed), then refusals and frames (IMG-PNG-3, IMG-PNG-8, IMG-JPG-1, IMG-RAS-1, IMG-RAS-2), then content (IMG-PNG-5, IMG-PNG-7, IMG-PNG-6, IMG-PNG-9, IMG-PNG-4, IMG-RAS-3, IMG-RAS-4, and the headline IMG-PNG-2), and the JPEG content rows last (IMG-PIX-1, IMG-JPG-5, IMG-JPG-2, IMG-JPG-6, IMG-JPG-3).

IMG-JPG-5 fails for a raster with a side above 65535 and IMG-PNG-7 for a byte list of 2^32 bytes or more (see Left to prove). IMG-JPG-3 and IMG-JPG-6 are false as worded: the round trip error reaches 7, and the colour conversion rounds green in two steps; both await a decision. IMG-JPG-2 covers every sampling layout the frame parser accepts, which BC-2 made decode correctly (REVIEW-13).
