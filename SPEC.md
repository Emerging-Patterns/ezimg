...
| IMG-PNG-2 | For every well-formed raster `r` with both sides nonzero and fewer than 2^29 samples, `decode_png(encode_png(r))` is `Some(r)`. | Proved | proved | LAWS.bend png_roundtrip_one; LAWS.bend png_roundtrip |
