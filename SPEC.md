...
| IMG-RAS-2 | For every well-formed raster `r` and every `x`, `y`: `get(r, x, y)` is `Some` of sample `y * w + x` when `x < w` and `y < h`, and none otherwise; `set(r, x, y, c)` changes exactly that sample to `c` when the point is inside and returns `r` unchanged otherwise. | Proved | pending | LAWS.bend get_inside; LAWS.bend get_inside_some; LAWS.bend get_outside; LAWS.bend set_inside; LAWS.bend set_outside |
| IMG-RAS-3 | For every well-formed `r` and rectangle `(x, y, cw, ch)`, `crop` is well formed, its size is the rectangle's intersection with `r` (0 by 0 when that is empty), and its sample at `(i, j)` is `r`'s sample at `(x + i, y + j)`. | Proved | pending | LAWS.bend crop_size; LAWS.bend crop_wf; LAWS.bend crop_at |
| IMG-RAS-4 | For every well-formed `dst` and `src` and offset `(x, y)`, `blit(dst, src, x, y)` has `dst`'s size, takes `src`'s sample at every point of `dst` that `src` covers, and leaves every other sample of `dst` unchanged. | Proved | pending | LAWS.bend blit_size; LAWS.bend blit_wf; LAWS.bend blit_in; LAWS.bend blit_out |
...
| IMG-RAS-2 | every raster whose `w * h` is below 2^32: inside, `get` is sample `y * w + x` (`get_inside`) and some when the raster is well formed (`get_inside_some`), and `set` replaces exactly that sample (`set_inside`); outside, `get` is none (`get_outside`) and `set` returns the raster (`set_outside`) for every raster | well-formed rasters with `w * h` of 2^32 or more, where the row is false as worded: `y * w + x` wraps in U32 (a decision for the maintainer) |
| IMG-RAS-3 | the size law for every raster (`crop_size`); well-formedness (`crop_wf`) and every sample (`crop_at`) for well-formed rasters whose `w * h` is below 2^32 | well-formed rasters with `w * h` of 2^32 or more, where the row is false as worded: the start index `y * w + x` wraps (a decision for the maintainer) |
| IMG-RAS-4 | the size law for every `dst` and `src` (`blit_size`); well-formedness (`blit_wf`), the covered samples (`blit_in`) and the others (`blit_out`) for well-formed `dst` and `src` whose `dst` has `w * h` below 2^32 | well-formed `dst` with `w * h` of 2^32 or more, where the row is false as worded: the row offsets `y * w` and `(y + rows) * w` wrap (a decision for the maintainer) |
