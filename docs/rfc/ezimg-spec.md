...
- [x] <!-- REVIEW-13 (resolved): IMG-JPG-2's wording. BC-2 found the cause of F-2: a block's row in its MCU was bi div vi where T.81 A.2.3 orders blocks hi to a row, so bi div hi. With that one-line fix, every layout the frame parser accepts (factors 1, 2 or 4 per component) decodes within 3 levels of `djpeg -nosmooth`: 1x1, 2x1, 2x2, 1x2, 4x1, 1x4, 4x2, 2x4 luma, and chroma at 1x1, 1x2, 2x1 or 2x2. Before it, every layout but 1x1 and 2x2 was off by up to 255. The decided row refuses all but 1x1, 2x1 and 2x2 luma with 1x1 chroma, so it would now refuse files ezimg reads correctly. Options: (a) reword IMG-JPG-2 to "for a SOF0 frame whose components' sampling factors are each 1, 2 or 4, every sample `decode_jpeg` returns is the frame's sample for that point, each component's samples replicated over the pixels its sampling covers, and `decode_jpeg` returns none for any other factor"; (b) keep the decided wording and add the refusal. Recommend (a): it removes nothing that works, and it states what the code now does. Decided: accepted as recommended, (a); IMG-JPG-2 is reworded. -->
...
| BC-4 (landed): `premultiply` and `straight` removed | none | 4 | callers of two identity functions |
...
| 2 (done) | SPEC.md with every row pending; all 85 closed laws, `spots` and law-only markers deleted; `closed` and `trace` at error; the README's Compliance section points at SPEC.md | the gate says exactly what is proved (nothing yet) |
