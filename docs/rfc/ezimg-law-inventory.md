...
| WP8, JPEG numeric rows | done (partial) | IMG-JPG-2 partly proved: `jpeg_refuse_factor1` and `jpeg_refuse_factor3` (a SOF0 segment with a factor outside 1, 2 and 4 makes `decode_jpeg` none, after any prefix that leaves the marker walk at a segment boundary, whatever follows), `jpeg_mcu_grid` (T.81 A.2.3 block order and sample size for every factor pair), `jpeg_block_cover` (a block's samples replicated over `pw` by `ph` pixels); lemmas `walk_app`, `walk_stop`, `fac_elim` in `proof/wp8-jpeg-numeric.bend`. Code: the factor and component-count checks are Bools of `U32.is_eq` (`decode.fac.ok`, `decode.fac.pair`, `decode.nf.ok`), `Comps` carries an `ok` Bool, and `decode.read.sof.c` refuses on the components first; probe outputs identical. Mutants (factor 3 accepted, V ignored, `bi div vi`, cursor bounds swapped) each fail a law. IMG-JPG-6 and IMG-JPG-3 are false as worded: `Jpeg.rgb` differs from exact T.871 on 3,320,385 of 2^24 inputs (by 1), and the JPEG round trip reaches 7 levels (a 2 by 2 raster gives 7); both wait on a maintainer decision |
